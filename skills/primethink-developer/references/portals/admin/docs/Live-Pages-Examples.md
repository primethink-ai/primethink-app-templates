...
async function applyFilters(resetPage = true) {
    ...
    if (statusFilter === 'pending') {
        ...
        // Filter server-side so pagination metadata matches the visible tasks
        serverFilters.completed = { $ne: true };
        serverFilters.due_date = { $lt: new Date().toISOString().split('T')[0] };
    }
    ...
    try {
        ...
        const filteredTasks = result.entities.filter(entity => entity.entity_name === 'task');
...
async function clearCompletedTasks() {
    // Page through all completed tasks - a single request is capped,
    // so keep fetching until no more pages remain
    ...
    while (hasMore) {
        ...
        completedTasks.push(...result.entities.filter(entity => entity.entity_name === 'task'));
...
async function loadNotes() {
    ...
    document.getElementById('notesList').innerHTML = notes.map(note => `
        ...
        >
            <div class="font-semibold">${escapeHtml(note.data.title || 'Untitled')}</div>
...
async function loadContacts() {
    ...
    document.getElementById('contactsGrid').innerHTML = contacts.map(contact => `
        <div class="bg-white rounded-lg shadow p-4">
            <h3 class="font-semibold text-lg">${escapeHtml(contact.data.name)}</h3>
            <p class="text-sm text-gray-600">${escapeHtml(contact.data.email || '')}</p>
            <p class="text-sm text-gray-600">${escapeHtml(contact.data.phone || '')}</p>
            <p class="text-sm text-gray-500">${escapeHtml(contact.data.company || '')}</p>
    ...
    const fileList = Array.from(files).map(file => {
        ...
        return `
            <div class="flex items-center gap-2 text-sm text-gray-600 bg-gray-50 p-2 rounded">
                ...
                <span class="flex-1">${escapeHtml(file.name)}</span>
...
function updateActionsLog() {
    ...
    logDiv.innerHTML = actionsLog.map(log => `
        <div class="flex items-start gap-3 p-3 bg-gray-50 rounded">
            <div class="flex-1">
                <div class="font-medium text-sm">${escapeHtml(log.action)}</div>
                <div class="text-sm text-gray-600">${escapeHtml(log.details)}</div>
            </div>
...
function cancelPending() {
    if (currentAbortController) {
        ...
        // Aborting only stops the client from waiting - also stop the
        // AI generation server-side so the request does not keep running.
        // Only clear currentTaskId once the server-side stop succeeds,
        // and surface a failure instead of silently discarding it.
        if (currentTaskId) {
            const taskId = currentTaskId;
            pt.stopStreamingMessage(taskId)
                .then(() => {
                ...
                .catch(error => {
                    showError('Cancelled locally, but stopping the AI generation failed: ' + error.message);
                });
        }
...
function displayResponse(question, message) {
    ...
    if (message.reasoning_steps && message.reasoning_steps.length > 0) {
        reasoningContent.innerHTML = message.reasoning_steps
            .map(step => `<div><strong>${escapeHtml(step.label)}:</strong> ${escapeHtml(step.content || '')}</div>`)
            .join('');
...
- `pt.stopStreamingMessage()` stops the server-side AI generation on cancel
...
[
  {
    ...
    "extraction_status": "<success|error>"
            ...
            try {
              ...
              // Upload failed after the row was created: mark the placeholder
              // row as error so it is not left in PROCESSING forever
              await pt.edit(entityId, { upload_status: 'error' }, true).catch(() => {});
              throw err;
            }
          })();
        }).filter(Boolean);
