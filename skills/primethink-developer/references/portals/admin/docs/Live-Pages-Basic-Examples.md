...
async function incrementCounter() {
    // Bounded retries with backoff instead of unbounded recursion
    for (let attempt = 0; attempt < 5; attempt++) {
        const counter = await pt.get(counterId);
        const newValue = counter.data.value + 1;

        // Optimistic locking: only save if no one else changed the counter since we read it
        const result = await pt.edit(counterId, { value: newValue }, false, counter.updated_at);
        if (!result.conflict) {
            updateDisplay(newValue);
            return;
        }
        // Another user updated the counter first - back off briefly, then retry with the latest value
        await new Promise(resolve => setTimeout(resolve, 100 * (attempt + 1)));
    }
    alert('Could not update the counter due to concurrent updates. Please try again.');
}
...
async function vote(optionId) {
    ...
    for (let attempt = 0; attempt < 5; attempt++) {
        ...
        // Optimistic locking: only save if no one else voted since we read the count
        ...
        // Another vote landed first - back off briefly, then retry with the latest count
        await new Promise(resolve => setTimeout(resolve, 100 * (attempt + 1)));
    }

    alert('Could not register your vote due to concurrent updates. Please try again.');
}
        ...
        <div class="bg-gray-50 p-3 rounded">
            ...
            ${Array.from(files).map(file => `
                <div class="text-sm text-gray-600">
                    ${escapeHtml(file.name)} (${(file.size / 1024).toFixed(2)} KB)
                </div>
            `).join('')}
        </div>
    `;
