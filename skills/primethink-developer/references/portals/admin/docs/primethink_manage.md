        ...
        async function loadMatters() {
            try {
                ...
                matters.matters.forEach(matter => {
                    // Use textContent (not innerHTML) for external data to avoid XSS
                    const div = document.createElement('div');
                    const title = document.createElement('h3');
                    title.textContent = matter.title;
                    const ref = document.createElement('p');
                    ref.textContent = `Ref: ${matter.ref_no}`;
                    const status = document.createElement('p');
                    status.textContent = `Status: ${matter.status.value}`;
                    div.append(title, ref, status);
