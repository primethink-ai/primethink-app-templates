...
async function renderMemberList() {
    ...
    const html = members.map(member => {
        ...
        const details = member.email ? `<small>${escapeHtml(member.email)}</small>` : '';

        return `
            <div class="member-card ${member.type}">
                ${icon} <strong>${escapeHtml(member.name)}</strong> ${badge}
                ${details}
            </div>
        `;
    }).join('');
        ...
        return `
            <div class="bg-white rounded-lg shadow p-4 mb-2">
                <div class="flex justify-between items-start">
                    <div>
                        <p class="font-medium">${escapeHtml(task.data.text)}</p>
